...
// 清空屏幕并显示LCD的ID和PWM_GEN按钮
void Load_Drow_Dialog(void)
{
	LCD_Clear(WHITE);								  // 清屏
	POINT_COLOR = BLUE;								  // 设置字体为蓝色
	LCD_ShowString(160, 400, 200, 32, 32, "PWM_GEN"); // 显示清屏区域
	LCD_ShowString(10, 10, 240, 16, 16, lcd_id); 		// 显示LCD ID
...
void ctp_test(void)
{
	...
	while (1)
	{
		tp_dev.scan(0);		//! 电容屏，scan函数在gt9147里（u8 GT9147_Scan(u8 mode)）
		for (t = 0; t < maxp; t++)
		{
			if ((tp_dev.sta) & (1 << t))	// 检查第t个点是否按下
			{
				if (tp_dev.x[t] < lcddev.width && tp_dev.y[t] < lcddev.height)	// 判断点击坐标是否合理
				{
					...
					if (tp_dev.x[t] < (300) && tp_dev.x[t] > (150) && tp_dev.y[t] > 400 && tp_dev.y[t] < 500)
					{
						...
						{
							PWM_Flag = 0;
							HAL_TIM_PWM_Stop(&TIM3_Handler,TIM_CHANNEL_1);	//! 关闭PWM通道1
...
int main(void)
{
	...
	TIM3_PWM_Init(500 - 1, 108 - 1);	//! 初始化TIM3的PWM
